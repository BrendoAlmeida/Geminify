...
interface TrackSearchCandidate {
  uri: string;
  title: string;
  artist: string;
  album?: string;
...
interface UnresolvedTrackSelection {
  index: number;
  requested: Song;
  searchQuery: string;
...
interface TrackResolutionChoice {
  index: number;
  selectedUri?: string | null;
}

interface GeminiTrackResolutionResponse {
...
interface PlaylistTrackFeature {
  id: string | null;
  uri: string | null;
  name: string;
  artist: string;
...
interface MixSummary {
  ...
  energy: { start: number; peak: number; end: number };
  key: { families: string[] };
  updatedAt: string;
}
...
function sanitizeChatPlaylistContext(raw: unknown): ChatPlaylistContext | undefined {
...
async function generateChatSuggestion(
  ...
  modelName?: string,
  playlistContext?: ChatPlaylistContext | null
): Promise<{ reply: string; themeTags: string[]; songExamples: string[] }> {
  if (!messages.length) {
...
app.post("/chat-ideas", async (req: Request, res: Response) => {
  try {
...
function extractArtistTokens(artist: string | undefined | null): string[] {
  if (!artist) {
    return [];
  }
...
function trackMatchesRequested(
...
function buildCandidateFromTrack(
  track: SpotifyApi.TrackObjectFull | null | undefined
...
async function findTrackUris(
  ...
  const queriesForSong = (song: { title: string; artist: string }): string[] => {
    const rawQueries = [
  ...
  for (let index = 0; index < songs.length; index += 1) {
    ...
    const result = await requestQueue.add(async () => {
      ...
      for (let attempt = 0; attempt < queries.length; attempt += 1) {
        ...
        try {
          const searchResponse = await exponentialBackoff(() =>
          ...
          const matched = items.find(
            (item: SpotifyApi.TrackObjectFull | null | undefined) =>
          ...
          const candidates = items
            .map((item: SpotifyApi.TrackObjectFull) =>
              buildCandidateFromTrack(item)
            )
            .filter(
              (candidate: TrackSearchCandidate | null): candidate is TrackSearchCandidate =>
                Boolean(candidate)
            )
            .slice(0, 10);
...
async function resolveMissingTracksWithGemini(
...
`;
  ...
  const response = await generateGeminiJson<GeminiTrackResolutionResponse>(prompt, modelName);
    ...
    for (const playlist of playlists) {
      try {
        ...
        const { uris, unresolved } = await findTrackUris(
          sanitizedPlaylist.songs,
          sanitizedPlaylist.name
        );

        if (unresolved.length) {
          log(
            `Requesting Gemini assistance for ${unresolved.length} unresolved tracks in "${sanitizedPlaylist.name}".`
          );
          ...
          if (resolvedCount > 0) {
            log(
              `Gemini resolved ${resolvedCount} tracks for "${sanitizedPlaylist.name}".`
            );
          }
          ...
          if (remaining > 0) {
            log(
              `${remaining} tracks remain unresolved for "${sanitizedPlaylist.name}".`
            );
          }
        }

        const validTrackUris = uris.filter(
          (uri: string | undefined): uri is string => typeof uri === "string" && Boolean(uri)
        );
        log(
...
app.get("/playlist-details", async (req: Request, res: Response) => {
  ...
  try {
    ...
    while (offset < totalTracks && songs.length < 400) {
      ...
      const batch = await requestQueue.add(() =>
        spotifyApi.getPlaylistTracks(playlistId, { offset, limit: batchLimit })
      );
...
app.post("/mix-playlist", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!isOwnedByUser) {
      return res.status(403).json({
        error:
          "We can only mix playlists that you created. Pick one you own or copy it to your profile first.",
      });
    }
    ...
    if (statusCode === 403) {
      return res.status(403).json({
        error:
          "Spotify wouldn't let us reorder that playlist. Make sure you own it and granted the latest permissions, then try again.",
      });
    }
...
${formatted}`);
    ...
    if (unresolved.length) {
      ...
      if (remaining > 0) {
        log(
          `${remaining} tracks remain unresolved after Gemini review for "${sanitizedPlaylist.name}".`
        );
      }
    ...
    const uniqueTrackUris = Array.from(new Set<string>(validTrackUris));
...
const CAMELOT_KEY_MAP: Array<{ major: string; minor: string }> = [
  { major: "8B", minor: "5A" },
  { major: "3B", minor: "12A" },
  { major: "10B", minor: "7A" },
  { major: "5B", minor: "2A" },
...
): Promise<PlaylistTrackFeature[]> {
  const collected: PlaylistTrackFeature[] = [];
  const totalTracks = playlist.tracks?.total ?? 0;
  const initialItems = playlist.tracks?.items ?? [];
  const limit = 100;

  const pushItem = (
    item: SpotifyApi.PlaylistTrackObject | null | undefined,
    ...
    const feature: PlaylistTrackFeature = {
      id: track.id ?? null,
      uri: track.uri ?? null,
      name: track.name,
      artist: artistName,
...
async function hydrateAudioFeatures(
  tracks: PlaylistTrackFeature[]
): Promise<void> {
  if (!tracks.length) {
    return;
  }
  ...
  const featureMap = uniqueIds.length
    ...
    : new Map<string, SpotifyApi.AudioFeaturesObject>();

  const tempoValues: number[] = [];
  const energyValues: number[] = [];
  const danceValues: number[] = [];
  ...
  for (const track of tracks) {
    ...
    if (Number.isFinite(track.tempo)) tempoValues.push(track.tempo);
    if (Number.isFinite(track.energy)) energyValues.push(track.energy);
    if (Number.isFinite(track.danceability)) danceValues.push(track.danceability);
    if (Number.isFinite(track.valence)) valenceValues.push(track.valence);
    if (Number.isFinite(track.loudness)) loudnessValues.push(track.loudness);
  }
  ...
  for (const track of tracks) {
    if (!Number.isFinite(track.tempo)) track.tempo = avgTempo;
    if (!Number.isFinite(track.energy)) track.energy = avgEnergy;
    if (!Number.isFinite(track.danceability)) track.danceability = avgDance;
    if (!Number.isFinite(track.valence)) track.valence = avgValence;
    if (!Number.isFinite(track.loudness)) track.loudness = avgLoudness;
    if (!Number.isFinite(track.timeSignature)) track.timeSignature = 4;
    if (!track.camelot) track.camelot = toCamelotInfo(null, null);
  }
}

async function fetchAudioFeatures(
  trackIds: string[]
): Promise<Map<string, SpotifyApi.AudioFeaturesObject>> {
  const result = new Map<string, SpotifyApi.AudioFeaturesObject>();
  const chunkSize = 100;
...
function toCamelotInfo(
  ...
  const letter = (label.endsWith("B") ? "B" : "A") as "A" | "B";

  return { label, number, letter };
}

function camelotDistance(a?: CamelotInfo | null, b?: CamelotInfo | null): number {
  if (!a || !b) {
    return 2;
  }
...
function targetEnergyForPosition(position: number, total: number): number {
  if (total <= 1) {
    return 0.55;
  }
...
function computeTransitionPenalty(
  current: PlaylistTrackFeature,
  next: PlaylistTrackFeature,
  targetEnergy: number,
  position: number,
  total: number
): number {
...
function buildMixedOrder(
  ...
  const ordered: PlaylistTrackFeature[] = [startTrack];
  ...
  for (let position = 1; position < total; position += 1) {
    ...
    let bestCandidate: PlaylistTrackFeature | null = null;
    let bestScore = Number.POSITIVE_INFINITY;

    for (const candidate of remaining) {
      const score = computeTransitionPenalty(
        current,
        candidate,
        targetEnergy,
        position,
...
function buildMixSummary(tracks: PlaylistTrackFeature[]): MixSummary {
...
async function applyPlaylistOrder(
  playlistId: string,
  originalOrder: PlaylistTrackFeature[],
  desiredOrder: PlaylistTrackFeature[]
): Promise<void> {
  ...
  for (let targetIndex = 0; targetIndex < desiredOrder.length; targetIndex += 1) {
    ...
    const currentIndex = working.findIndex(
      (track) => track.originalIndex === targetTrack.originalIndex
    );
    ...
    await requestQueue.add(() =>
      spotifyApi.reorderTracksInPlaylist(playlistId, currentIndex, targetIndex, {
        range_length: 1,
      })
    );
    ...
    log(
      `Moved track "${targetTrack.name}" from position ${currentIndex + 1} to ${
        targetIndex + 1
      }`
    );
